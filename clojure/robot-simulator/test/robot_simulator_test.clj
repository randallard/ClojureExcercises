...
(deftest can-move-north
  (is (= {:x 0 :y 1} (robot-simulator/simulate-move :north))))

(deftest can-get-new-coords
  (def coords {:x 5 :y 4})
  (def xy-transform (robot-simulator/simulate-move :north))
  (is (= {:x 5 :y 5} {:x (+ (:x coords) (:x xy-transform)) :y (+ (:y coords) (:y xy-transform))})))
