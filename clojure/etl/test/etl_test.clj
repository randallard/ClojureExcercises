...
(deftest returns-a-hash-map-of-element-value
  (is (= {"element" 1}
         (etl/transform {1 ["element"]}))))

(deftest returns-a-hash-map-of-two-elements-and-value
  (is (= {"element" 1 "another element" 1}
         (etl/transform {1 ["element" "another element"]}))))

(deftest returns-a-hash-map-from-two-elements-two-values
  (is (= {"a" 1 "b" 1 "c" 2 "d" 2}
         (etl/transform {1 ["a" "b"]
                         2 ["c" "d"]}))))

(deftest keys-are-lower-case
  (is (= {"a" 1}
         (etl/transform {1 ["A"]}))))
